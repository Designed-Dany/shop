...
const getItemSubTotalPrice = (inputs) => Math.floor(Number(inputs.value) * Number(inputs.dataset.price)) // сокращенная версия 

const getItemOldTotalPrice = (inputs) => Math.floor(Number(inputs.value) * Number(inputs.dataset.oldprice))
...
function hiddenCartText() {
	checkboxText.textContent = [...document.querySelectorAll('.product__item_active')].length + ' товара  · ' + totalPriceWrapper.textContent + ' сом'
